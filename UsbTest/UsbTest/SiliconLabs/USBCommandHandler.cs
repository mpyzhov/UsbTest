...
namespace UsbTest
{
    ...
    {
        private const string DllName = "SiUSBXp.dll";
        ...
        public static string SI_GetVID(uint dwDeviceNum)
        {
            return SI_GetProductString(dwDeviceNum, SI_RETURN_VID);
        }

        public static string SI_GetPID(uint dwDeviceNum)
        {
            return SI_GetProductString(dwDeviceNum, SI_RETURN_PID);
        }

        public static string SI_GetDeviceName(uint dwDeviceNum)
        {
            return SI_GetProductString(dwDeviceNum, SI_RETURN_DESCRIPTION);
        }

        public static string SI_GetDeviceSerialNumber(uint dwDeviceNum)
        {
            return SI_GetProductString(dwDeviceNum, SI_RETURN_SERIAL_NUMBER);
        }

        private static string SI_GetProductString(uint dwDeviceNum, byte dwFlag)
        {
            StringBuilder data = new StringBuilder();
            if (SI_GetProductString(dwDeviceNum, data, dwFlag) != SI_SUCCESS)
            {
                return string.Empty;
            }

            return data.ToString();//ASCIIEncoding.ASCII.GetString(data).Trim('\0');
        }
